...
def filter_single_line(line, index, args, fo, ferror, forbidden_urls):
...
def filter_one_file(file_path, output_file_path, args):
    ...
    n_success=0
    n_filtered=0
    with gzip.open(file_path, 'rt', encoding='utf-8') as f, gzip.open(output_file_path, 'wt', encoding='utf-8') as fo, open("error.json","wt") as ferror:
        ...
        try:
            ...
            for index, line in enumerate(pbar):
                success=filter_single_line(line, index, args, fo, ferror, forbidden_urls_trie)
                if success:
                    n_success+=1
                else:
                    n_filtered+=1
                # pbar.set_postfix_str(f"{file_name} success:filtered={n_success}:{n_filtered}")
                if index%10000==0:
                    # pbar.set_postfix_str(f"{file_name} success:filtered={n_success}:{n_filtered}")
                    print(f"{file_name} success:filtered={n_success}:{n_filtered}")
        except Exception as e:
            print(e)
        print(f"finish to process {file_path}")
...
def filter_dataset(args):
    ...
    for file_path in file_paths:
        ...
        pool.apply_async(filter_one_file, (file_path, output_path, args))
        # filter_one_file(file_path, output_path, args)
    pool.close()
    pool.join()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--fasttext_model_dir", default='lid.176.bin', help="Fasttext model directory, wget from https://dl.fbaipublicfiles.com/fasttext/supervised-models/lid.176.bin")
    parser.add_argument("--cn_bad_words_dir", default='cn_bad_words.txt', help="Another chinese bad words list file path.")

    parser.add_argument("--input_dir", default='../c4/multilingual',help="Input file directory.")
    parser.add_argument("--output_dir", default='../c4/multilingual_clean',help="Output file directory.")
    parser.add_argument("--workers", default=16, type=int, help="Multiprocessing workers' num.")
    args = parser.parse_args()
    
    if not os.path.exists(args.fasttext_model_dir):
        print("Please download from https://dl.fbaipublicfiles.com/fasttext/supervised-models/lid.176.bin")
        exit()
